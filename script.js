document.addEventListener('DOMContentLoaded', () => {
    const API_URL = window.location.origin + '/simple/api/'; 
    ...
    async function fetchData(entity, params = {}) {
        ...
        if (isBaseRequest && Array.isArray(state[entity]) && state[entity].length > 0) {
            return state[entity];
        }
        ...
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            try {
                ...
                if (result.success) {
                    ...
                    // Clear the cache for this entity to force a refetch
                    state[entity] = null;
    ...
    async function loadDashboardContent() {
        ...
        if (role === 'admin') {
            ...
            try {
                const mahasiswaData = await fetchData('mahasiswa');
                const dosenData = await fetchData('dosen');
                const matakuliahData = await fetchData('matakuliah');
                
                console.log('Mahasiswa data:', mahasiswaData);
                console.log('Dosen data:', dosenData);
                console.log('Matakuliah data:', matakuliahData);
                
                const mahasiswaCount = mahasiswaData.length;
                const dosenCount = dosenData.length;
                const matakuliahCount = matakuliahData.length;
                ...
                mainContent.innerHTML = '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">Error loading dashboard data</div>';
            }
    ...
    async function setupPresensiPage() {
        ...
        document.getElementById('add-btn').addEventListener('click', renderAdminInputPresensi); // Admin can also input
    ...
    async function renderAdminPresensiView() {
        ...
        document.getElementById('input-presensi-btn').addEventListener('click', () => loadPageContent('input_presensi'));
